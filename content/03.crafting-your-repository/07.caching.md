...
Turborepo 的缓存在本地工作时可以显著节省时间 - 当启用[远程缓存](/docs/core-concepts/remote-caching)时更加强大，可以在整个团队和 CI 之间共享缓存。

在本页面中，您将学习：

- [如何命中您的第一个 Turborepo 缓存](#hit-your-first-turborepo-cache)
- [如何启用远程缓存](/docs/core-concepts/remote-caching)
- [Turborepo 用于哈希的输入和输出内容](/docs/crafting-your-repository/caching#task-inputs)
...
如果您已经[全局安装了 `turbo`](/docs/getting-started/installation#global-installation)，请在您的仓库中运行 `turbo build`。
...
这将导致缓存未命中，因为您从未在此仓库中使用此[输入集合](/docs/crafting-your-repository/caching#task-inputs)运行过 `turbo`。输入被转换为哈希，用于在本地文件系统缓存或[远程缓存](/docs/core-concepts/remote-caching)中检查。
...
要了解更多关于远程缓存及其优势的信息，请访问[远程缓存页面](/docs/core-concepts/remote-caching)。
...
有关如何将 CI 机器连接到远程缓存的信息，请访问[构建 CI 指南](/docs/crafting-your-repository/constructing-ci#enabling-remote-caching)。

::alert{type="info"}
默认情况下，Turborepo 使用 [Vercel 远程缓存](https://vercel.com/docs/monorepos/remote-caching)，无需配置。如果您想使用不同的远程缓存，请访问[远程缓存 API 文档](/docs/core-concepts/remote-caching#self-hosting)
::
...
Turborepo 缓存在 `turbo.json` 的 [`outputs` 键](/docs/reference/configuration#outputs)中定义的任务文件输出。当缓存命中时，Turborepo 将从缓存中恢复文件。

`outputs` 键是可选的，请参阅 [API 参考](/docs/reference/configuration#outputs)了解 Turborepo 在这种情况下的行为。
...
您可以使用 [`--output-logs` 标志](/docs/reference/run#--output-logs-option)或 [`outputLogs` 配置选项](/docs/reference/configuration#outputlogs)配置重放日志的详细程度。
...
| 输入                                                                                  | 示例                                                                                                                                                    |
| -------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 从根 `turbo.json` 和包 `turbo.json` 解析的任务定义         | 在根 `turbo.json` 或[包配置](/docs/reference/package-configurations)中更改 [`outputs`](/docs/reference/configuration#outputs) |
| 影响工作区根的锁文件更改                                        | 在根 `package.json` 中更新依赖项将导致**所有**任务未命中缓存                                                                        |
| [`globalDependencies`](/docs/reference/configuration#globaldependencies) 文件内容 | 当 `./.env` 在 `globalDependencies` 中列出时更改它将导致**所有**任务未命中缓存                                                         |
| [`globalEnv`](/docs/reference/configuration#globalenv) 中列出的变量值   | 当 `GITHUB_TOKEN` 在 `globalEnv` 中列出时更改其值                                                                                      |
...
| 输入                                                                   | 示例                                                 |
| ----------------------------------------------------------------------- | ------------------------------------------------------- |
| [包配置](/docs/reference/package-configurations)更改 | 更改包的 `turbo.json`                       |
...
Turborepo 有一个 [`--dry` 标志](/docs/reference/run#--dry----dry-run)，可用于查看如果您运行任务会发生什么，而无需实际运行它。这对于调试缓存问题很有用，当您不确定正在运行哪些任务时。

有关更多详细信息，请访问 [`--dry` API 参考](/docs/reference/run#--dry----dry-run)。

### 使用运行摘要

Turborepo 有一个 [`--summarize` 标志](/docs/reference/run#--summarize)，可用于获取任务的所有输入、输出等的概览。比较两个摘要将显示为什么两个任务的哈希不同。这对以下情况很有用：
...
有时，您可能不希望将任务的输出写入缓存。这可以使用 [`"cache": false`](/docs/reference/configuration#cache) 为任务永久设置，或使用 [`--cache <options>` 标志](/docs/reference/run#--no-cache)为整个运行设置。

### 覆盖缓存

如果您想强制 `turbo` 重新执行已缓存的任务，请使用 [`--force` 标志](/docs/reference/run#--force)。请注意，这会禁用**读取**缓存，**而不是写入**。

### 缓存任务比执行任务慢

可能会创建缓存最终比不缓存更慢的场景。这些情况很少见，但一些示例包括：

- **执行极快的任务**：如果任务的执行速度比到[远程缓存](/docs/core-concepts/remote-caching)的网络往返更快，您应该考虑不缓存该任务。
