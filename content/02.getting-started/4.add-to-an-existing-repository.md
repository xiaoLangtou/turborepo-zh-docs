...
安装 `turbo` 并在 `turbo.json` 中配置任务后，您会发现[缓存](/crafting-your-repository/caching)如何帮助您更快地运行任务。
...
要了解更多关于 Turborepo 在单包工作空间中的使用，请访问[专门的指南](/single-package-workspaces)。
...
::callout{type="info"}
如果您发现 `turbo` 遇到问题，比如无法发现工作空间中的包或不遵循依赖图，请访问我们的[构建仓库结构](/crafting-your-repository/structuring-a-repository)页面获取提示。
::
...
要了解更多关于我们为什么推荐两种安装方式的信息，请访问[安装页面](/getting-started/installation)。
...
有关配置 `turbo.json` 的更多信息，请参阅[配置选项](/api-reference/configuration/configuring-turbo.json)文档。
...
::callout{type="info"}
根据您的仓库情况，在迁移过程中或在无法使用 `packageManager` 键的情况下，您可能需要使用 [`dangerouslyDisablePackageManagerCheck`](/api-reference/configuration/configuring-turbo.json#dangerouslydisablepackagemanagercheck)。
::
...
有关如何构建仓库结构的更多详细信息，请参阅[构建仓库结构](/crafting-your-repository/structuring-a-repository#declaring-directories-for-packages)。
...
这会同时运行 `build` 和 `check-types` 任务。您的[工作空间](/crafting-your-repository/structuring-a-repository#anatomy-of-a-workspace)的依赖图将用于按正确顺序运行任务。
...
要了解更多关于 `turbo` 如何实现这一点的信息，请查看[缓存文档](/crafting-your-repository/caching)。
...
您还可以[使用过滤器](/crafting-your-repository/running-tasks#using-filters)专注于特定包及其依赖项。
...
- [为开发机器启用远程缓存](/crafting-your-repository/constructing-ci#enabling-remote-caching)
- [在 CI 中启用远程缓存](/crafting-your-repository/constructing-ci)
- [处理环境变量](/crafting-your-repository/using-environment-variables)
- [过滤任务](/api-reference/commands/run#--filter-string)
